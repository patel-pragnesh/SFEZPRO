...
function populateMenuItems(data) {
	var dataRow = [];
	for (var i = 0; i < data.length; i++) {
		Ti.API.info("*********populateMenuItems***********inside loop************* " + data[i].title + " " + data[i].title + " " + data[i].price.value + " " + data[i].description);
		...
		var menuItemImg = Ti.UI.createImageView({
			right : 10,
			height : 35,
			width : 70,
			top : 25,
			image : "/images/consumerordering/sample_menu_item.png"
		});
